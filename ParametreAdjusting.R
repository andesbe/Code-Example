...
#Sorting out the outliers, as they are of little interest to our case
InputOutput <- cbind(input, output)
InputOutput <- as.data.frame(subset(InputOutput, InputOutput[,24] <= 300))
...
model <-keras_model_sequential()%>%
  layer_dense(units = 110, activation = "relu", input_shape =c(4),  kernel_regularizer = regularizer_l1(l = 0.0005))%>%
  # layer_dropout(0.15) %>%
...
EvalIn <- as.matrix(read.csv(file="plotInEval3.csv"))
EvalOut <- as.matrix(read.csv(file="plotOutEval3.csv"))

mean(EvalIn[,4])


EvalInputOutput <- cbind(EvalIn, EvalOut)
EvalInputOutput <- as.data.frame(subset(EvalInputOutput, EvalInputOutput[,24] <= 300))


EvalIn <- as.data.frame(rbind(EvalInputOutput[,1:4]))
EvalOut <- as.data.frame(rbind(EvalInputOutput[,5:25]))

#Scaling the new data using the distribution of the training dataset
...
mse1 <- abs(mean((as.vector(pred_new[,1]) -as.vector(y.eval[,1]))/as.vector(pred_new[,1])))
mse2 <- abs(mean((as.vector(pred_new[,2]) -as.vector(y.eval[,2]))/as.vector(pred_new[,2])))
mse3 <- abs(mean((as.vector(pred_new[,3]) -as.vector(y.eval[,3]))/as.vector(pred_new[,3])))
mse4 <- abs(mean((as.vector(pred_new[,4]) -as.vector(y.eval[,4]))/as.vector(pred_new[,4])))
mse5 <- abs(mean((as.vector(pred_new[,5]) -as.vector(y.eval[,5]))/as.vector(pred_new[,5])))
mse6 <- abs(mean((as.vector(pred_new[,6]) -as.vector(y.eval[,6]))/as.vector(pred_new[,6])))
mse7 <- abs(mean((as.vector(pred_new[,7]) -as.vector(y.eval[,7]))/as.vector(pred_new[,7])))
mse8 <- abs(mean((as.vector(pred_new[,8]) -as.vector(y.eval[,8]))/as.vector(pred_new[,8])))
mse9 <- abs(mean((as.vector(pred_new[,9]) -as.vector(y.eval[,9]))/as.vector(pred_new[,9])))
mse10 <- abs(mean((as.vector(pred_new[,10]) -as.vector(y.eval[,10]))/as.vector(pred_new[,10])))
mse11 <- abs(mean((as.vector(pred_new[,11]) -as.vector(y.eval[,11]))/as.vector(pred_new[,11])))
mse12 <- abs(mean((as.vector(pred_new[,12]) -as.vector(y.eval[,12]))/as.vector(pred_new[,12])))
mse13 <- abs(mean((as.vector(pred_new[,13]) -as.vector(y.eval[,13]))/as.vector(pred_new[,13])))
mse14 <- abs(mean((as.vector(pred_new[,14]) -as.vector(y.eval[,14]))/as.vector(pred_new[,14])))
mse15 <- abs(mean((as.vector(pred_new[,15]) -as.vector(y.eval[,15]))/as.vector(pred_new[,15])))
mse16 <- abs(mean((as.vector(pred_new[,16]) -as.vector(y.eval[,16]))/as.vector(pred_new[,16])))
mse17 <- abs(mean((as.vector(pred_new[,17]) -as.vector(y.eval[,17]))/as.vector(pred_new[,17])))
mse18 <- abs(mean((as.vector(pred_new[,18]) -as.vector(y.eval[,18]))/as.vector(pred_new[,18])))
mse19 <- abs(mean((as.vector(pred_new[,19]) -as.vector(y.eval[,19]))/as.vector(pred_new[,19])))
mse20 <- abs(mean((as.vector(pred_new[,20]) -as.vector(y.eval[,20]))/as.vector(pred_new[,20])))
